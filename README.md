...
Agent 只能依据用户材料组织内容。证据不足处应明确标记“待补充”，不得编造指标、客户、职责、结果或个人贡献。默认删除内部链接、用户明细、密钥、项目代号和未经确认可披露的精确业务数据。公开示例统一使用中性业务描述，并将可反向识别的规模、比例与排名改写为“明显提升”“可观比例”“大规模覆盖”等范围表达，同时明确标注“具体数据已脱敏”，保留结果口径、采用事实和贡献边界。
