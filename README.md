...
打开[在线 Demo 的即时诊断](https://haimuhaimu.github.io/strategy-product-portfolio-template/#instant-diagnostic)，粘贴一段项目经历，即可在浏览器本地检查结果证据、口径、方法、资产和贡献边界。原文不会上传，输出只包含覆盖情况、常见隐私风险数量和最优先的补证据问题。诊断完成后还可以下载本地生成的 PNG 安全分享卡；卡片不含输入原文，只展示总分与等级、五维覆盖、隐私风险数量、唯一优先追问和体验入口。
