  ...
  return (
    <main className="bg-[#efefe9] text-[#14110e]">
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-8 sm:py-20">
        <p className="font-mono text-xs font-bold tracking-[0.18em] text-[#c92a20]">展示结构库 / v0.6.2</p>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          {TEMPLATE_REGISTRY.map((template, index) => (
            <article key={template.id} id={template.id} className="border border-[#14110e]/20 bg-[#fffdf8] p-5 shadow-[7px_7px_0_rgba(20,17,14,0.08)] sm:p-7">
              ...
              <p className="mt-5 text-base font-semibold leading-7">{template.tagline}</p>
              <p className="mt-3 text-sm leading-7 text-[#5b4635]"><strong className="text-[#14110e]">适合：</strong>{template.audience}</p>
              <p className="mt-3 text-sm leading-7 text-[#5b4635]"><strong className="text-[#14110e]">招聘官先看：</strong>{template.recruiterFirstLook}</p>
              <p className="mt-4 border-l-2 border-[#c92a20] pl-4 text-sm font-semibold leading-7">{template.focus}</p>
              ...
              <div className="mt-6"><h3 className="font-mono text-xs font-bold text-[#80654d]">选择信号</h3><ul className="mt-3 flex flex-wrap gap-2">{template.matchSignals.map((signal) => <li key={signal} className="bg-[#f4dfbd] px-2 py-1 text-xs text-[#5b4635]">{signal}</li>)}</ul></div>
              <StaticPageLink href={`/templates/${template.id}/`} className="mt-7 inline-flex items-center border-b border-[#14110e] pb-1 text-sm font-semibold transition hover:text-[#c92a20]">
                查看详细说明 →
              </StaticPageLink>
            </article>
          ))}
