  ...
  return (
    <main className="bg-[#efefe9] text-[#14110e]">
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-8 sm:py-20">
        <p className="font-mono text-xs font-bold tracking-[0.18em] text-[#c92a20]">TEMPLATE LIBRARY / v0.5</p>
        <div className="mt-5 grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
          <div><h1 className="max-w-5xl font-serif text-4xl font-semibold leading-tight sm:text-6xl">模板不是换皮，而是决定招聘官先看到哪条证据链。</h1><p className="mt-6 max-w-3xl text-base leading-8 text-[#5b4635]">四个模板读取同一份 v2 projects.json，但首页与项目页的信息顺序、区块命名和叙事入口不同。Launchpad 只按已提供的内容信号评分，不补写事实。</p></div>
          ...
          {TEMPLATE_REGISTRY.map((template, index) => (
            <article key={template.id} id={template.id} className="border border-[#14110e]/20 bg-[#fffdf8] p-5 shadow-[7px_7px_0_rgba(20,17,14,0.08)] sm:p-7">
              ...
              <div className={`mt-6 border p-4 ${previewTone[template.id as keyof typeof previewTone]}`}>
                <p className="font-mono text-[10px] font-bold tracking-[0.12em]">STRUCTURE PREVIEW</p>
              ...
              <div className="mt-6 grid gap-5 sm:grid-cols-2"><div><h3 className="font-mono text-xs font-bold text-[#80654d]">项目页顺序</h3><ol className="mt-3 space-y-2 text-sm text-[#5b4635]">{template.projectStructure.map((item, itemIndex) => <li key={item}>{itemIndex + 1}. {item}</li>)}</ol></div><div><h3 className="font-mono text-xs font-bold text-[#80654d]">匹配信号</h3><ul className="mt-3 flex flex-wrap gap-2">{template.matchSignals.map((signal) => <li key={signal} className="bg-[#f4dfbd] px-2 py-1 text-xs text-[#5b4635]">{signal}</li>)}</ul></div></div>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="bg-[#14110e] px-4 py-14 text-[#f8f8f3] sm:px-8">
        <div className="mx-auto flex max-w-7xl flex-wrap items-end justify-between gap-7"><div><p className="font-mono text-xs font-bold text-[#d3b992]">NEXT / LOCAL ONLY</p><h2 className="mt-3 max-w-3xl text-3xl font-semibold sm:text-4xl">导入自己的 projects.json，查看四模板可解释评分。</h2><p className="mt-4 max-w-2xl text-sm leading-7 text-[#d3b992]">选择结果只在 Release Pack 的 projects.json 写入 template.active；隐私和引用检查仍是硬阻断。</p></div><StaticPageLink href="/launchpad/" className="bg-[#c92a20] px-6 py-3 font-semibold text-white transition hover:bg-[#e13b30]">进入 Launchpad →</StaticPageLink></div>
      </section>
    </main>
  );
}
