...
const architectureBySlug: Record<string, ArchitectureConfig> = {
  "creator-monetization-health": {
    ...
    stages: [
      { label: "输入", title: "多源变现行为", body: "商单、交易、内容付费、服务、平台激励" },
  ...
  "marketing-commerce-traffic-system": {
    title: "优质营销流量实验架构",
    thesis: "不是凭经验给商单加流量，而是用实验验证优质作者是否值得被放大。",
    stages: [
      { label: "输入", title: "作者价值体系 2.0", body: "内容质量、商业适配、历史表现、用户反馈" },
      { label: "分层", title: "分层与对照", body: "优质作者池、实验分组、护栏指标" },
      { label: "实验", title: "双场景实验", body: "商单与交易内容分别验证收益" },
      { label: "动作", title: "流量机制化", body: "分层倾斜、优质加权、反馈调优" },
      { label: "资产", title: "商单流量标准", body: "从一次实验变成可复用的分发机制" },
    ],
    feedback: "实验结果证明的是优质作者池的收入提升，不是全量作者收入提升。",
  },
  "paid-content-evaluation-typing": {
    ...
    stages: [
      ...
      { label: "流量", title: "自动化投流", body: "冷启动验证、推荐放大、效果观察、复盘提醒" },
  ...
  "image-text-recommendation-strategy": {
    ...
    stages: [
      ...
      { label: "场景", title: "三类场景拆解", body: "单列主 feed、双列发现、UGC 图文社区" },
      { label: "判断", title: "独立价值信号", body: "阅读、停留、滑动、收藏、搜索承接、负反馈" },
      { label: "动作", title: "分发与冷启策略", body: "单列建模、双列候选、社区首刷承接" },
      { label: "验证", title: "规模化验证", body: "主站 DAU、频道 DAU、社区 DAU 分别证明" },
    ],
  ...
  "game-content-distribution": {
    ...
    feedback: "这条链路主要靠内容带发行，不是纯广告投流；流水口径也必须能明确归因。",
  },
