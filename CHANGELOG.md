...
即时证据诊断新增“一键复制可转发链接（不含原文）”。分享结果采用带版本号的严格白名单 JSON，经 UTF-8 Base64URL 编码后仅写入 URL Fragment；只包含总分、等级、五维覆盖、隐私风险数量和安全白名单中的唯一追问，不使用 query、服务端 API、网络请求或新增依赖。非法、字段越界、非规范编码、被篡改或超长载荷会安全拒绝。

接收者打开链接会直接看到“朋友分享的安全诊断结果”及本地运行声明，可通过“我也测一下”清除分享状态与 Hash，并回到聚焦后的输入框。PNG 分享卡与安全链接均基于当前 origin 和 GitHub Pages basePath 动态生成，支持 fork、子路径及自定义域名；同时保持纯静态导出、移动端、键盘、`aria-live` 与 `prefers-reduced-motion` 体验。

公开模板内容完成全局脱敏：业务专名与内部表达改为中性、可理解的公开口径，可反向识别的规模、比例和排名改为大范围、显著增量、可观比例或行业领先，并保留“具体数据已脱敏”的证据边界。公开安全测试同步扩展到源码、数据、文档、示例、schema、测试夹具与 `public` 文本资源；通过受控扫描范围和技术夹具允许列表避免误伤版本号等正常技术信息。版本保持 `0.9.1`。

## 2026-08-21 — v0.9.0

即时证据诊断新增“下载安全分享卡”：分享卡由纯函数生成 SVG，再由浏览器 Canvas 转换为 PNG，全程不上传内容、不请求服务端。卡片仅包含总分与等级、五维覆盖、隐私风险数量、唯一优先追问、产品名与体验链接，以及本地运行和不含原文声明。

首页新增 3 组面向产品经理与运营的匿名 Before / After 教学案例，强调对象、判断、动作、口径与贡献边界的结构性改写，并支持一键载入诊断。新增首批 20 位免费体验招募区，提供短版、社群版与私聊邀请版一键复制；明确不上传经历、不替用户编造结果。保持纯静态导出、移动端与无障碍反馈，未新增依赖。
